...
const PROJECTS = [
  { key: '1', year: '2025', image: '/works/aesthetics.jpg' },
  { key: '2', year: '2025', image: '/works/startup.jpg' },
  { key: '3', year: '2024', image: '/works/opera.jpg' },
  { key: '4', year: '2024', image: '/works/bakery.jpg' },
]
...
export default function WorksSection() {
        ...
        gsap.fromTo(card,
          { rotateY: 8, scale: 0.9, opacity: 0.4 },
          {
            ...
            scrollTrigger: {
              ...
              start: 'left 90%', end: 'left 40%', scrub: 1,
            },
          }
        )
        gsap.to(card, {
          ...
          scrollTrigger: {
            trigger: card, containerAnimation: tween,
            start: 'right 60%', end: 'right 10%', scrub: 1,
          },
        })
      })
    }, section)
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.set(titleRef.current, { yPercent: 100, opacity: 0, filter: 'blur(14px)', visibility: 'hidden' })
      gsap.set(subtitleRef.current, { yPercent: 50, opacity: 0, filter: 'blur(8px)', visibility: 'hidden' })

      const tl = gsap.timeline({
        scrollTrigger: { trigger: section, start: 'top 80%', toggleActions: 'play none none none' },
      })

      tl.to(labelRef.current, { opacity: 1, y: 0, visibility: 'visible', duration: 0.6, ease: 'power3.out' }, 0)
      tl.to(titleRef.current, { yPercent: 0, opacity: 1, filter: 'blur(0px)', visibility: 'visible', duration: 1.2, ease: 'power4.out' }, 0.1)
      tl.to(subtitleRef.current, { yPercent: 0, opacity: 1, filter: 'blur(0px)', visibility: 'visible', duration: 1.0, ease: 'power3.out' }, 0.3)
    }, section)
      ...
      <div ref={pinRef} className="works-pin">
        ...
        <div className="works-progress">
          <div className="works-progress-track">
            <div className="works-progress-fill" />
          </div>
          <span className="works-progress-text">{tr('works.egor', lang)}</span>
        </div>
      </div>
    </section>
  )
